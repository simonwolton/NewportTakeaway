...
if ($_SERVER["REQUEST_METHOD"] == "POST") // If anything was posted here, do this...
{
	...
	if($validationPassed == true)
	{
		...
		$connection = mysql_connect("exampleDBaddress", "exampleDBusername", "exampleDBpassword");
		mysql_select_db("newport_takeaway")or die("cannot select DB");
		$result = mysql_query("INSERT INTO items (itemName, itemDescription, itemPrice) VALUES ('" . $itemName . "','" . $itemDescription . "','" . $itemPrice . "');");
...
if ($done == true)
{
	$formContents = "<span style=\"margin-left:95px;color:green;\">Menu item added!</span>";
...
$formContents = <<< EOPAGE
	<form name="addMenuItemForm" method="post" action="$sendToSelf"> 
		$generalErr
		<div class="align">
