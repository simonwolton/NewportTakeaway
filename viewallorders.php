		...
		<div id="wrapper">
			...
			<div id="main">
				<div id="content">
					...
					$allOrdersQR = mysql_query("SELECT * FROM `orders` WHERE date IS NOT NULL AND orderComplete IS NULL ORDER BY date ASC");
					
					if (mysql_num_rows($allOrdersQR) == 0)
						echo "No orders not completed!<br /><br /><br />";
					else
					{
						echo "Orders not yet dispatched, oldest first:";
						echo "<table id=\"basicTable\" style=\"margin:0 auto;width:800px;\" >";
						echo "<tr>";
						echo "<th>Order #</th>";
						echo "<th>User</th>";	
						echo "<th>Date</th>";
						echo "<th style='width:100px;'>Total (&pound;)</th>";
						echo "<th>Done?</th>";
						echo "</tr>";

						while($order = mysql_fetch_array($allOrdersQR))
						{
							echo "<tr onclick=\"window.location.href = 'vieworder.php?ID=$order[orderID]';\">";
							echo "<td> " . $order["orderID"] . "</td>";
							$row = mysql_fetch_assoc(mysql_query("SELECT FirstName,LastName FROM `users` WHERE UserID = $order[userID]"));
							echo "<td> " . $row['FirstName'] . " " . $row['LastName'] . "</td>";
							echo "<td> " . $order["date"] . "</td>";
							echo "<td> <span style='color:green;display:block;text-align:right;'>" . number_format($order["orderPrice"],2) . "</td>";
							echo "<td><a href='ordercomplete.php?ID=$order[orderID]'><img src='images/check.png'/></a></td>";
							echo "</tr>";
						}
						echo "</table>";
					}
					
					$allOrdersQR = mysql_query("SELECT * FROM `orders` WHERE date IS NULL ");
					echo "Ongoing orders (not yet checked out):";
					echo "<table id=\"basicTable\" style=\"margin:0 auto;width:800px;\" >";
					echo "<tr>";
					echo "<th>Order #</th>";
					echo "<th>User ID</th>";
					...
					$allOrdersQR = mysql_query("SELECT * FROM `orders` WHERE date IS NOT NULL AND orderComplete IS NOT NULL ORDER BY date ASC");
