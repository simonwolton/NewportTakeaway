...
{
	die("You can't create an account whilst logged in!<br /><a href='logout.php'>Logout first</a>");
}
// define variables and set to empty values
$firstNameErr = $lastNameErr = $emailErr = $passwordErr = $passwordReentryErr = $addressOneErr = $addressTwoErr = $postcodeErr = "";
$firstName = $lastName = $email = $password = $passwordReentry = $addressOne = $addressTwo = $postcode = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") // If anything was posted here, do this...
{
	...
	if ($validationPassed == true)
	{
		...
		$doesEmailExist = mysql_query("SELECT EmailAddr FROM users WHERE EmailAddr = '$email'");
		
		$row = mysql_fetch_array($doesEmailExist);
		if ($row > 0)
		{
			$formContents = "<p>That email exists on our system! Try logging in instead.</p>";
			$formContents .= '<a href="login.php" style="color:#596772;">Login</a> or <a href="createaccount.php" style="color:#596772;">Try Again</a><br />';
		}
		else
		{
			mysql_query("INSERT INTO users(FirstName,LastName,EmailAddr,Password,AddressOne,AddressTwo,Postcode) VALUES ('" . $firstName . "','" . $lastName . "','" . $email . "','" . md5($password) ."','" . $addressOne ."','" . $addressTwo ."','" . $postcode ."')");
			mysql_close($connection);
			$formContents = "User has been created!";
...
$formContents = <<< EOPAGE
	<form name="accountForm" method="post" action="$sendToSelf"> 
		...
		<!--<div class="align">
			<label for="passwordReentry">Re-Type Password:</label>
			<input class="inputalign" type="password" id="passwordReentry" name="passwordReentry" value="$passwordReentry"/><span class="error">$passwordReentryErr</span>
		</div>-->
		<div class="align">
